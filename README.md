...
**Fills into loops.** Hold a chorus and the repeat used to just restart. Now, when a pass is about to loop, the kit stems jump to a bar where *this drummer filled on this recording* while the band keeps playing the bar as written — same kit, same room, same take — and everything lands together on the downbeat. Fills are found automatically (`tools/find_fills.py` scores every bar for hit density, weight in the second half of the bar and skins-over-cymbals) and stored as `audio.fills`. Checkbox in the panel; splices are crossfaded.

**Getting stems.** `tools/prepare_song.py` is the pipeline: an **ensemble** of two Demucs architectures (`htdemucs_ft` + `hdemucs_mmi`) averaged together — different models make different mistakes, and averaging cancels much of what makes a single model sound "EQ'd" — then the ensemble's `other` is divided into guitar / piano / rest using only the 6-source model's *ratios* (`tools/split_other.py`, a Wiener-style split that sums back exactly, so the guitar inherits the good model's quality), then DrumSep for the kit parts, then bar times and fills from the drum stem, then MP3s into `local/audio/<song-id>/` and the song JSON updated. Older maskers (Spleeter and most free web tools) only decide which frequencies to keep, which is why they sound like an EQ of the mix. Paid services (Moises, AudioShake, SpectraLayers, RipX) can additionally split lead from rhythm guitar, which no open model does yet; whatever they hand you, drop the files in with **Load stems…** and each becomes its own fader (names come from the filenames: `drums`, `bass`, `vocals`, `guitar`, `lead`, `rhythm`, `piano`, `kick`, `snare`…).
...
**Live vocal pitch correction** (Mixer → each channel → *Pitch fix*). Real-time, in an AudioWorklet on your mic: YIN pitch detection every 12 ms, correction toward the nearest note of the song's key (or a key you pick, or chromatic), and TD-PSOLA resynthesis — grains one period long re-laid at the corrected period — so the formants never move and the voice keeps its timbre. **Retune** is the natural-versus-robot knob: 100–300 ms lets vibrato and slides through and just tightens note centres; 10–30 ms is the hard-tune sound. **Amount** sets how far toward the note it pulls. Unvoiced sounds and silence pass straight through. Fixed latency 26 ms (plus your interface); the strip shows the detected pitch and how far it's being moved. *Follow song* takes the key from the chart's first chord and tracks any key change you make in the Backing panel. Measured cost: about 14 % of one core on this project's slow test machine for a single channel — expect a few percent on an M2.

**Drum punch** (fx → *Punchy drums* preset, or the *Drum punch* effect on kick/snare when the kit is split): a transient shaper in its own worklet — fast and slow envelope followers; attacks get boosted, the body gets tightened — plus a snap tilt EQ, all under one intensity knob. **Reverb** now offers room / plate / hall / spring (generated, nothing to download) with pre-delay, and any `.wav` you drop in `local/ir/` appears in the list.
