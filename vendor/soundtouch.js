 ...
 * (StageDrums: contains a small modification in Stretch.process() for guided offsets — see the comment there.)
...
class Stretch extends AbstractFifoSamplePipe {
  // StageDrums: guide = offsets to replay, record = array to fill (see process())
  guide = null; guideIndex = 0; record = null;
  constructor(createBuffers) {
    super(createBuffers);
  ...
  process() {
    ...
    while (this._inputBuffer.frameCount >= this.sampleReq) {
      // StageDrums modification (LGPL-2.1, see soundtouch.LICENSE): "guided" WSOLA. When `guide` is set, reuse the
      // overlap offsets recorded from a reference pass instead of seeking, so several stems processed with the same
      // guide make identical time-domain decisions and stay sample-aligned with each other. `record` collects them.
      if (this.guide && this.guideIndex < this.guide.length) offset = this.guide[this.guideIndex++];
      else offset = this.seekBestOverlapPosition();
      if (this.record) this.record.push(offset);
